...
\begin{figure}[h]
	...
	\resizebox{0.9\textwidth}{!}{%
		\begin{tikzpicture}[
			...
			% Nodes
			\node[box, fill=green!20] (user) {User actions \\ (forms + file upload)};
			\node[box, fill=blue!20, right=of user] (entities) {Domain entities \\ (Project, Proposal, \\Sample, Experiment)};
			\node[box, fill=orange!20, right=of entities] (db) {Database \\ (PostgreSQL)};
			\node[box, fill=gray!20, below=of entities] (raw) {Raw bucket \\ (TIFFs + README)};
			\node[box, fill=gray!20, right=of raw] (mirror) {Mirror bucket \\ (NeXus + checksums)};
			
			% Arrows
			\draw[->] (user) -- (entities) node[midway, above] {create/register};
			\draw[->] (entities) -- (db) node[midway, above] {metadata};
			\draw[->] (entities) -- (raw) node[midway, left] {upload files};
			\draw[->] (raw) -- (mirror) node[midway, below] {derived data};
...
\begin{figure}[h]
	...
	\resizebox{0.9\textwidth}{!}{%
		\begin{tikzpicture}[
			...
			% Nodes
			\node[box, fill=gray!20] (tiff) {Raw TIFF file(s)};
			\node[box, fill=blue!20, right=2.5cm of tiff] (extract) {Metadata extraction \\ (TIFF headers, JSON blocks)};
			\node[box, fill=orange!20, below=of extract] (mapping) {Mapping layer \\ (JSON mapping file)};
			\node[box, fill=yellow!20, right=2.5cm of mapping] (normalize) {Normalization \\ (units, datatypes, defaults)};
			\node[box, fill=green!20, below=of mapping] (nexus) {NXem / NeXus file structure};
			
			% Arrows
			\draw[->] (tiff) -- (extract) node[midway, above] {read headers};
			\draw[->] (extract) -- (mapping) node[midway, right] {source keys};
			\draw[->] (mapping) -- (nexus) node[midway, right] {target fields};
			\draw[->] (mapping.east) -- (normalize.west) node[midway, above] {apply rules};
			\draw[->] (normalize.south) |- (nexus.east) node[midway, above] {clean values};
...
\paragraph{c) Instrument-specific population.}
...
\paragraph{d) Mapping mechanics (example).}
...
\paragraph{g) Resulting file layout (as written).}
...
\paragraph{j) Traceability and repeat-safety.}
...
\begin{figure}[h]
	...
	\resizebox{0.9\textwidth}{!}{%
		\begin{tikzpicture}[
			...
			% User/browser
			\node[box, fill=green!20] (browser) {User in browser \\ (HTMX modal / Uppy uploader)};
			
			% Django app
			\node[box, fill=blue!20, below=of browser] (django) {Django view \\ (domain command + validation)};
			
			% Redis and workers (left side)
			\node[box, fill=orange!20, below left=2.2cm and 2.5cm of django] (redis) {Redis Queue (RQ) \\ job queue};
			\node[box, fill=gray!20, below=of redis] (workers) {RQ Workers \\ (Python processes)};
			
			% Background tasks (center)
			\node[box, fill=yellow!20, below=3.5cm of django] (tasks) {Background tasks \\ 
				- compute\_checksum \\ 
				- make\_nexus\_from\_tiff};
			
			% Database and storage (bottom layer)
			\node[box, fill=purple!10, below left=2.5cm and 1.5cm of tasks] (db) {PostgreSQL database};
			\node[box, fill=purple!15, below right=2.5cm and 1.5cm of tasks] (s3) {Object storage \\ (Ceph RGW / MinIO)};
			
			% Arrows
			\draw[->] (browser) -- (django) node[midway, right] {form submit / upload};
			\draw[->] (django.west) .. controls +(left:2cm) and +(up:1cm) .. (redis.north) node[midway, above left] {enqueue job};
			\draw[->] (redis) -- (workers);
			\draw[->] (workers.east) -- (tasks.west) node[midway, right] {execute};
			\draw[->] (django) -- (tasks.north) node[midway, right] {register metadata};
			\draw[->] (tasks.south west) -- (db.north) node[midway, left] {write metadata};
			\draw[->] (tasks.south east) -- (s3.north) node[midway, right] {upload files, checksums};
			
		\end{tikzpicture}%
	}
	\caption{Flow of background tasks. An action in the browser (via HTMX or Uppy) calls a Django view, which validates input and enqueues jobs into Redis RQ. Worker processes execute tasks such as computing checksums or building NeXus files, writing results to the database and object storage.}
	\label{fig:background-tasks-flow}
\end{figure}
...
\section{User interface walkthrough}\label{sec:ui-screenshots}
